...
void updateRangeUtil(int actual, int a_l, int a_r, int update_l, int update_r, int cambio){
  if (a_l>update_r || a_r<update_l) //Si esta fuera del rango
        return;
...
int getSumUtil(int a_l, int a_r, int q_l, int q_r, int actual){
  if (a_l>q_r || a_r<q_l) //Si esta fuera del rango
		return 0;
...
int main(){
  ...
  for(int i = 0; i < num_casos; i++){
      ...
      int n = cadena.length();
      constructSTUtil(cadena, 0, n-1, 0);
      ...
      for (int j = 0; j < num_operaciones; j++) {
          ...
          cin >> l >> r;
          if(operacion == 'F'){
              updateRangeUtil(0, 0, n-1, l, r, 1);

            }

          if(operacion == 'E'){
              updateRangeUtil(0, 0, n-1, l, r, 3);

            }

          if(operacion == 'I'){
              updateRangeUtil(0, 0, n-1, l, r, 2);

            }

          if(operacion == 'S'){
              cout << getSumUtil(0, n-1, l, r, 0) << '\n';
